              ...
              filtered.map((q) => (
                <tr key={q.id}>
                  ...
                  <td className="flex gap-2">
                    <button className="btn btn-sm btn-info" onClick={() => setEditTarget(q)}>
                      <Pencil className="w-4 h-4" />
                    </button>
                    <button className="btn btn-sm btn-error" onClick={() => setDeleteTarget(q)}>
      ...
      {showAddToQuizModal && (
        <CreateQuizFromQuestionsModal
          ...
          onAddToExisting={({ quizId, questions }) => {
            console.log(`Add to quiz ${quizId}:`, questions);
            setShowAddToQuizModal(false);
          }}
